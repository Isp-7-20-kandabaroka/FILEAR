...
async def process_ad_description(message: types.Message, state: FSMContext):
    # Фильтруем текст на наличие запрещенных слов и контактной информации
    filtered_description = filter_description(message.text)

    # Проверяем длину фильтрованного описания
    if len(filtered_description) > 1024:
        # Если фильтрованное описание слишком длинное, просим пользователя ввести его заново
        await message.answer("Описание слишком длинное. Пожалуйста, укоротите его до 1024 символов и попробуйте снова.")
    else:
        # Сохраняем корректное описание в состояние
        async with state.proxy() as data:
...
async def show_ad(user_id, ad, state: FSMContext):
    ...
    # Проверка и сокращение длины подписи, если необходимо
    if len(message_text) > 1024:
        # Сокращаем текст, оставляя место для "..."
